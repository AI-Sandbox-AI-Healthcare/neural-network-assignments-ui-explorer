...
tbody td.cond-col{max-width:420px}
...
<div id="main">
  ...
  <div class="tab-pane active" id="tab-overview">
    ...
    <div class="info-grid">
      <div class="info-card">
        <h3>🏥 The Problem</h3>
        <p>Identify patients with chronic pain from their electronic health records (EHR). Chronic pain
...
const CONCEPTS = [
  ...
  },
  {
    ...
    explanation:`Our features have wildly different scales: age ranges 0–90, healthcare expenses range 0–500,000. Without scaling, large-scale features can dominate the gradient, making the model pay much more attention to expenses than age.\n\nZ-score standardization subtracts the feature mean and divides by the feature standard deviation. This puts each feature on a comparable scale, with mean around 0 and standard deviation around 1. For roughly normal features, most values will fall near \[-3, 3\], although outliers can go beyond that range. This helps the optimizer update model weights more evenly across features.\n\nCritically: you compute mean and std on the training set ONLY, then apply those same values to the validation set. Otherwise you leak validation information into training.`,
...
function filterTable(){
  ...
  const rows = filtered.map((p)=>{
    ...
    return '<tr>' + cols.map((col,ci)=>{
      ...
      else if(col==='condition_text'){
        const full = p[col]||'';
        val = `<span title="${full.replace(/"/g,"'")}">` + full.slice(0,110) + (full.length>110?'…':'') + '</span>';
      }
      else if(typeof val==='number') val = Number.isInteger(val) ? val : val.toFixed(2);
      const colCls = (ci===0?'first-col ':'') + (col==='condition_text'?'cond-col':'');
      return `<td class="${colCls}">${val??''}</td>`;
    }).join('') + '</tr>';
  }).join('');
